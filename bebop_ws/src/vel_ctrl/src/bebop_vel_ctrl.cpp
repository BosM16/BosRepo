    ...
    beb_vy_pred_m_(0.0)
{
  ...
  util::GetParam(nh_priv_, "min_alt", param_min_alt_, 0.2);
                                     ...
                                     const bebop_msgs::Ardrone3PilotingStateSpeedChangedConstPtr& speed_ptr)
{
  ...
  if (!beb_param_recv)
  {
    ...
    if (!util::GetParam(nh_, "/bebop/bebop_driver/PilotingSettingsMaxTiltCurrent", beb_maxtilt_rad_))
    {
      return;
    }
    beb_maxtilt_rad_ = angles::from_degrees(beb_maxtilt_rad_);
    if (!util::GetParam(nh_, "/bebop/bebop_driver/SpeedSettingsMaxVerticalSpeedCurrent", beb_max_speed_vert_m_))
    {
      return;
    }
    if (!util::GetParam(nh_, "/bebop/bebop_driver/SpeedSettingsMaxRotationSpeedCurrent", beb_max_speed_rot_rad_))
    {
      return;
    }
  ...
  ROS_DEBUG_STREAM("[VCTL] Current Bebop Velocities: XYZ: "
                  << beb_vx_m_ << " "
                  << beb_vy_m_ << " "
                  << beb_vz_m_ << " ");
