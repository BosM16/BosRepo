...
subplot(211), plot(time, velocity_x, time, velocity_x_filt),title('vel_{x,filt}')
subplot(212), plot(time, velocity_y, time, velocity_y_filt),title('vel_{y,filt}')
...
y = velocity_y(2:end-1);
Phi = [-velocity_y(1:end-2), input(2:end-1), input(1:end-2)];
theta = Phi\y;
...
y2 = velocity_y_filt(2:end-1);
Phi2 = [-velocity_y_filt(1:end-2), input_filt(2:end-1), input_filt(1:end-2)];
theta_filt = Phi2\y2;
...
y_pk = velocity_y_filt(2:end-1);
Phi_pk = [-velocity_y_filt(1:end-2), input_filt(2:end-1)];
...
subplot(212)
plot(t,velocity_y - x_pk)
title('Difference between simulation and measurement')
legend('vel_{y,meas}-vel_{y,sim}')
xlabel('Time [s]')
ylabel('Displacement [m]')
axis tight
